...
def logit_compare(n,b,p,q,p1s):
    ...
    isl_data_logit = np.empty((5,len(p1s)))
    gen_data_logit = np.empty((5,len(p1s)))

    for i in range(len(p1s)):
        p1 = p1s[i]
        isl_data_logit[:,i] = sim.ko_test(procs=4,
        ...
        gen_data_logit[:,i] = sim.ko_test(procs=4,
                                          seeds=seeds[(i*b):(i*b+b)],
                                          q=q,
                                          knockoff='binary',
                                          model='logit',
    ...
    # save data 
    fg = open('data/logit_genetic.json','w')
    json.dump(gen_data_logit,fg)
    fg.close()

    fi = open('data/logit_ising.json','w')
    json.dump(isl_data_logit,fi)
    fi.close()
