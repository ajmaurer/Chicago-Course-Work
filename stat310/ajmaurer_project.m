...
if prob(1)
    ...
    % of y above tau to 0 and above tau to 1 to getxtau (I store all 100 vectors 
...
if prob(3) 
    ...
    % Convert matricies so this can be formatted as a QP/LP. Matricies are huge but sparse
    % both times, the objective is a norm of Cx
    xs = sparse(reshape(S.*X,[m*n,1]))      % elements of X, or 0 if S is 0 (data missing)
    ...
    % optimize the problem, in this case a QP
    cvx_begin
        variable x(m*n)
        minimize(norm(C*x,2))
        subject to 
...
if prob(4) 
    ...
    % Usual least squares for the first k data points, constraint that a'x>beta for remainder
    cvx_begin
        variable x_qp(d)
    ...
    re_lp = norm(x_ls - x_true)/norm(x_true)
